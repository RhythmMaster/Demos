...
@end
static const CGFloat hz_defaultSpace = 8;
static char backClear_key, lineClear_key, myNavView_key;

@implementation UINavigationBar (TZLHandle)
...
-(void)sx_layoutSubviews{
    ...
    if (tCurrentDeviceVersion >= 11) {//需要调节
        self.layoutMargins = UIEdgeInsetsZero;
        CGFloat space = hz_defaultSpace;
...
- (void)navBarAlpha:(CGFloat)alpha isOpaque:(BOOL)opaque {
    ...
    self.myNavView.hiddenBottomLine = !alpha;
...
- (void)creatMainUIByFrame:(CGRect)frame {
    ...
    self.backImageView.backgroundColor = [[UIColor whiteColor] colorWithAlphaComponent:0.95];
